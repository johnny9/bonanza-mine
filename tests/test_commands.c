...
static void test_writereg(void) {
    ...
    // header32 + count + data
    TEST_ASSERT_EQUAL_size_t(4 + 1 + 3, m.last_len);
    TEST_ASSERT_EQUAL_HEX8(0x01, m.last_payload[0]);
    // opcode in high nibble (0x2)
    TEST_ASSERT_EQUAL_HEX8(0x20, (uint8_t)(m.last_payload[1] & 0xF0));
    TEST_ASSERT_EQUAL_HEX8(0x02, (uint8_t)(m.last_payload[1] & 0x0F));
    TEST_ASSERT_EQUAL_HEX8(0x34, m.last_payload[2]);
    TEST_ASSERT_EQUAL_HEX8(0x56, m.last_payload[3]);
    TEST_ASSERT_EQUAL_HEX8(2, m.last_payload[4]);
    TEST_ASSERT_EQUAL_HEX8(0xDE, m.last_payload[5]);
}
